...
encodeListWith : Context -> List Int -> Result Errors String
...
validInput : List Int -> Result Errors ()
validInput list =
    case list of
        [] ->
            Ok ()
...
noNumbers : Test
noNumbers =
    testEncodeWithTitle "If no numbers are passed, return an empty string" [] (Ok "")
...
defaultAlphabet =
    describe "Encode with default alphabet"
        [ testFn encodeList [ 0 ] (Ok "bM")
        , testFn encodeList [ 0, 1, 2 ] (Ok "rSCtlB")
        ]
...
testEncodeWithTitle : String -> List Int -> Result Errors String -> Test
testEncodeWithTitle title input expected =
    Test.test title <|
        \() ->
            encodeList input |> Expect.equal expected


testToId : Test
testToId =
    [ toId 1 (Array.fromList [ 'a', 'b', 'c' ]) |> Expect.equal "b"
    , toId 13 ("abcde" |> String.toList |> Array.fromList) |> Expect.equal "cd"
    ]
